...
```bash
composer require archilex/filament-filter-sets:"^5.0"
```
...
Before upgrading to Advanced Tables v5, first be sure you are familiar with Filament's [upgrade guide](https://filamentphp.com/docs/5.x/upgrade-guide) and know what changes will be needed along the way. 
...
1. Update the dependency
    ...
    > Note: If you are upgrading from Filament v4 then you may stop here. However, if you are updating from Filament v3 directly to v5, then you should continue with the steps below. You should also be familiar with Filament's v4 [upgrade guide](https://filamentphp.com/docs/4.x/upgrade-guide).
...
You can disable the Favorites Bar globally (helpful if you only want to use [reorderable columns](#reorderable-columns)) by passing `false` to the `favoritesBarEnabled()` method:
