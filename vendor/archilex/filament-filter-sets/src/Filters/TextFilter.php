...
class TextFilter extends BaseFilter
{
                ...
                $data['operator'] === TextOperator::IS
                    ...
                    function (Builder $query) use ($data, $queryColumn) {
                        ...
                        if (filled($this->getOptions())) {
                            return $query->when(
                                $this->isMultiple(),
                                fn (Builder $query) => $query->whereIn($queryColumn, $data['values']),
                                fn (Builder $query) => $query->where($queryColumn, $data['value']),
                            );
                        }
    ...
    public function getFormSchema(): array
    {
        ...
        $schema[] = TextInput::make('value')
            ->hiddenLabel()
            ->hidden(
                fn (\Filament\Schemas\Components\Utilities\Get $get) => blank($get('operator')) ||
