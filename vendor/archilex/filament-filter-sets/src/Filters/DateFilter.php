...
use Archilex\AdvancedTables\Filters\Concerns\HasEmptyQuery;
...
class DateFilter extends BaseFilter
{
    ...
    public function apply(Builder $query, array $data = []): Builder
    {
        ...
        return $query->whereHas(
            $this->column->getRelationshipName($query->getModel()),
            ...
            \Filament\Schemas\Components\Grid::make()
                ...
                ->schema([
                    ...
                    Select::make('operator')
                        ...
                        ->afterStateUpdated(function (\Filament\Schemas\Components\Utilities\Get $get, \Filament\Schemas\Components\Utilities\Set $set, ?string $state) {
                            if (
                    ...
                    \Filament\Schemas\Components\Grid::make()
                        ...
                        ->schema([
                            TextInput::make('value')
                                ...
                                ->visible(fn (\Filament\Schemas\Components\Utilities\Get $get) => in_array($get('operator'), [DateOperator::IN_THE_NEXT, DateOperator::IN_THE_LAST, DateOperator::EXACTLY, DateOperator::BEFORE, DateOperator::AFTER])),
                            TextInput::make('between_start')
                                ...
                                ->visible(fn (\Filament\Schemas\Components\Utilities\Get $get) => in_array($get('operator'), [DateOperator::BETWEEN])),
                            ...
                            Select::make('unit')
                                ...
                                ->columnSpan([
                                    'default' => fn (Livewire $livewire, \Filament\Schemas\Components\Utilities\Get $get) => in_array($get('operator'), [DateOperator::IN_THIS, DateOperator::IS_NEXT, DateOperator::IS_LAST, DateOperator::BETWEEN]) ? 4 : 3,
                                    'sm' => function (Livewire $livewire, \Filament\Schemas\Components\Utilities\Get $get) {
                                ...
                                ->visible(fn (\Filament\Schemas\Components\Utilities\Get $get) => in_array($get('operator'), [DateOperator::IN_THIS, DateOperator::IS_NEXT, DateOperator::IS_LAST, DateOperator::IN_THE_NEXT, DateOperator::IN_THE_LAST, DateOperator::EXACTLY, DateOperator::BEFORE, DateOperator::AFTER, DateOperator::BETWEEN])),
                            DatePicker::make('date_start')
                                ...
                                ->visible(fn (\Filament\Schemas\Components\Utilities\Get $get) => in_array($get('operator'), [DateOperator::IS_DATE, DateOperator::BEFORE_DATE, DateOperator::AFTER_DATE, DateOperator::BETWEEN_DATES])),
                            DatePicker::make('date_end')
                                ...
                                ->visible(fn (\Filament\Schemas\Components\Utilities\Get $get) => in_array($get('operator'), [DateOperator::BETWEEN_DATES])),
                        ])
                        ->hidden(function (\Filament\Schemas\Components\Utilities\Get $get) {
                            return
                                blank($get('operator')) ||
                                in_array($get('operator'), [DateOperator::YESTERDAY, DateOperator::TODAY, DateOperator::TOMORROW, DateOperator::IS_EMPTY, DateOperator::IS_NOT_EMPTY]);
                        })
                        ->columnSpan([
    ...
    protected function applyQuery(Builder $query, array $data, string $column): Builder
    {
        ...
        if ($this->isEmptyQuery($data)) {
            return $this->applyEmptyQuery($query, $data, $column);
        }
    ...
    protected function formFilled(array $data): bool
    {
        ...
        if (in_array($operator, [DateOperator::IN_THIS, DateOperator::IS_NEXT, DateOperator::IS_LAST]) && filled($unit)) {
            return true;
        }

        if (in_array($operator, [DateOperator::IN_THE_NEXT, DateOperator::IN_THE_LAST, DateOperator::EXACTLY, DateOperator::BEFORE, DateOperator::AFTER]) && filled($value) && filled($unit)) {
            return true;
        }

        if (in_array($operator, [DateOperator::BETWEEN]) && (filled($betweenStart) && filled($betweenEnd) && filled($unit))) {
            return true;
        }

        if (in_array($operator, [DateOperator::IS_DATE, DateOperator::BEFORE_DATE, DateOperator::AFTER_DATE]) && filled($dateStart)) {
            return true;
        }

        if (in_array($operator, [DateOperator::BETWEEN_DATES]) && (filled($dateStart) || filled($dateEnd))) {
            return true;
        }

        if (in_array($operator, [DateOperator::YESTERDAY, DateOperator::TODAY, DateOperator::TOMORROW, DateOperator::IS_EMPTY, DateOperator::IS_NOT_EMPTY])
        ) {
            return true;
        }
    ...
    protected function getOperators(): array
    {
        return [
            ...
            __('advanced-tables::filter-builder.form.empty') => [
                DateOperator::IS_EMPTY => __('advanced-tables::filter-builder.filters.date.is_empty.option'),
                DateOperator::IS_NOT_EMPTY => __('advanced-tables::filter-builder.filters.date.is_not_empty.option'),
            ],
        ];
    }
