...
trait HasFavoritesBar
{
    ...
    public static function getFavoritesBarSize(): null | string | \Filament\Support\Enums\Size
    {
        if (self::pluginRegistered()) {
            return AdvancedTablesPlugin::get()->getFavoritesBarSize();
        }

        return config('advanced-tables.favorites_bar.size', \Filament\Support\Enums\Size::Medium);
    }

    public static function favoritesBarHasDefaultView(): bool
    {
        if (self::pluginRegistered()) {
